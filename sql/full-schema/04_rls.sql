...
CREATE POLICY psl_select ON public.project_summary_links AS PERMISSIVE FOR SELECT TO anon, authenticated
  USING (true);
CREATE POLICY psl_write ON public.project_summary_links AS PERMISSIVE FOR ALL TO anon, authenticated
  USING ((jwt_claim('user_role'::text) = ANY (ARRAY['admin'::text, 'superadmin'::text])))
  WITH CHECK ((jwt_claim('user_role'::text) = ANY (ARRAY['admin'::text, 'superadmin'::text])));
