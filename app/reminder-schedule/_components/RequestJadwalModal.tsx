...
export function RequestJadwalModal({
  ...
  const handleSubmit = async () => {
    ...
    if (!form.address.trim()) { alert('Lokasi project wajib diisi!'); return; }
    if (!form.due_date) { alert('Tanggal wajib diisi!'); return; }
    setSubmitting(true);
    try {
      await onSubmit(form);
    } finally {
  ...
  return (
    <div
      className="fixed inset-0 bg-black/60 flex items-center justify-center z-[10000] p-4 overflow-y-auto"
