        ...
        <div className="p-6 space-y-4">
          ...
          <div className="p-4 rounded-xl space-y-3" style={{background:'#fffbeb',border:'1px solid #fde68a'}}>
            ...
            <div><label htmlFor="f-unit-movement-components-addeditmodal-2" className={lbl}>{isMasuk?'👤 Nama PTS (Penerima)':'👤 Nama PTS (Pengirim)'}</label>
              <select id="f-unit-movement-components-addeditmodal-2" className={inp+" cursor-pointer"} value={form.nama_pts} onChange={e=>set('nama_pts',e.target.value)}>
                <option value="">-- Pilih Anggota PTS --</option>
                {teamMembers.map(m=><option key={m} value={m}>{m}</option>)}
              </select>
            </div>
            <div><label htmlFor="f-unit-movement-components-addeditmodal-3" className={lbl}>{isMasuk?'🏢 Pengirim (Pihak Luar)':'🏢 Penerima (Pihak Luar)'}</label>
