...
export function MiniPieChart({
  ...
  if (total === 0) return (
    <div className="rounded-2xl p-2 sm:p-4 flex flex-col gap-1 sm:gap-2"
      style={{ background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(255,255,255,0.8)', backdropFilter: 'blur(10px)' }}>
      <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest">{icon} {title}</p>
      <p className="text-gray-400 text-xs sm:text-sm text-center py-2 sm:py-4">Belum ada data</p>
    </div>
  );
  ...
  return (
    <div className="rounded-2xl p-2 sm:p-4 flex flex-col gap-1 sm:gap-3"
      style={{ background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(255,255,255,0.8)', backdropFilter: 'blur(10px)' }}>
      <p className="text-[10px] sm:text-xs font-bold text-gray-600 uppercase tracking-widest">{icon} {title}</p>
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
        ...
        <svg aria-hidden="true" focusable="false" width="120" height="120" viewBox="0 0 120 120"
          className="flex-shrink-0 w-[68px] h-[68px] sm:w-[120px] sm:h-[120px]">
          {slices.map((s) => (
            s.isFullCircle ? (
        ...
        <div className="flex flex-col gap-1 sm:gap-1.5 flex-1 basis-[100px] min-w-[100px] sm:basis-[150px] sm:min-w-[150px] max-w-[210px] max-h-[80px] sm:max-h-[120px] overflow-y-auto">
          {slices.map((s) => {
            const isActive = activeFilter === s.label;
            return (
              <div key={s.i}
                className="flex items-center gap-1.5 cursor-pointer rounded-lg px-1.5 py-0.5 sm:py-0.5 transition-all"
                style={{
