      ...
      <div className="flex flex-wrap items-center justify-center gap-1 sm:gap-3">
        ...
        <div className="flex flex-col gap-0.5 sm:gap-1.5 flex-1 basis-[80px] min-w-[80px] sm:basis-[150px] sm:min-w-[150px] max-w-[210px] max-h-[68px] sm:max-h-[120px] overflow-y-auto">
          {slices.map((s) => {
            ...
            return (
              <div key={s.i}
                className="flex items-center gap-1 sm:gap-1.5 cursor-pointer rounded-lg px-1 sm:px-1.5 py-px sm:py-0.5 transition-all"
                style={{
                ...
                <span className="text-[9px] sm:text-[10px] font-semibold text-gray-600 truncate flex-1">{s.label}</span>
                <span className="text-[9px] sm:text-[10px] font-bold flex-shrink-0" style={{ color: s.color }}>{s.value}{valueSuffix ?? ''}</span>
